...
const QUICK_LABELS = ["Hole 1", "Hole 2", "Deep pool", "Riffle", "Bank spot", "Under bridge"];
...
const HoleNamingPrompt = ({ onConfirm, onCancel, holeCount }: HoleNamingPromptProps) => {
  const [label, setLabel] = useState(`Hole ${holeCount + 1}`);
  ...
  return (
    <div className="absolute bottom-4 left-4 right-4 z-50 bg-card border border-border rounded-xl p-3 shadow-lg space-y-2">
      ...
      <div className="flex gap-2 justify-end">
        ...
        <Button type="button" size="sm" className="rounded-xl gap-1" onClick={() => onConfirm(label || `Hole ${holeCount + 1}`)}>
